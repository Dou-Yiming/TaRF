...
@dataclass
class TaRFModelConfig(ModelConfig):
    """TaRF Model Config"""

    _target: Type = field(default_factory=lambda: TaRFModel)
    near_plane: float = 0.05
    """How far along the ray to start sampling."""
    far_plane: float = 1000.0
    """How far along the ray to stop sampling."""
    background_color: Literal["random",
                              "last_sample", "black", "white"] = "last_sample"
    ...
    proposal_net_args_list: List[Dict] = field(
        default_factory=lambda: [
            {"hidden_dim": 16, "log2_hashmap_size": 17,
                "num_levels": 5, "max_res": 128, "use_linear": False},
            {"hidden_dim": 16, "log2_hashmap_size": 17,
                "num_levels": 5, "max_res": 256, "use_linear": False},
        ]
    )
...
class TaRFModel(Model):
    """TaRF model

    Args:
        config: TaRF configuration to instantiate model
    """

    config: TaRFModelConfig
        ...
        **kwargs,
        ...
        def button_cb(button: ViewerButton):
            def click_cb(click: ViewerClick):
                ...
                camera.height = torch.tensor([[480]])
                camera.cx = torch.tensor([[camera.width//2]])
                camera.cy = torch.tensor([[camera.height//2]])
                camera.fx = camera.width/(2*np.tan(np.radians(fov)/2))
                camera.fy = camera.fx
    ...
    def populate_modules(self):
        ...
        if self.config.use_same_proposal_network:
            assert len(
                self.config.proposal_net_args_list) == 1, "Only one proposal network is allowed."
            ...
            for i in range(num_prop_nets):
                prop_net_args = self.config.proposal_net_args_list[min(
                    i, len(self.config.proposal_net_args_list) - 1)]
                network = HashMLPDensityField(
                    self.scene_box.aabb,
                    spatial_distortion=scene_contraction,
                    **prop_net_args,
        ...
        # None is for piecewise as default (see ProposalNetworkSampler)
