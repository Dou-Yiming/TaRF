...
class ResNetT2(pl.LightningModule):
    def __init__(self, name='resnet50', modality='touch'):
        super(ResNetT2, self).__init__()
        if name == 'resnet50':
            self.l_to_ab = resnet50(in_channel=3, width=1.0)
            self.ab_to_l = resnet50(in_channel=3, width=1.0)
        elif name == 'resnet18':
            self.l_to_ab = resnet18(in_channel=3, width=1.0)
            self.ab_to_l = resnet18(in_channel=3, width=1.0)
        elif name == 'resnet101':
            self.l_to_ab = resnet101(in_channel=3, width=1.0)
            self.ab_to_l = resnet101(in_channel=3, width=1.0)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))
        self.modality = modality
        assert self.modality in ['touch','image']
...
class ResNet_concat(pl.LightningModule):
    def __init__(self, name='resnet50', modality='touch', in_channel=3):
        super(ResNet_concat, self).__init__()
        if name == 'resnet50':
            self.l_to_ab = resnet50(in_channel=in_channel, width=1)
            self.ab_to_l = resnet50(in_channel=in_channel, width=1)
        elif name == 'resnet18':
            self.l_to_ab = resnet18(in_channel=in_channel, width=1)
            self.ab_to_l = resnet18(in_channel=in_channel, width=1)
        elif name == 'resnet101':
            self.l_to_ab = resnet101(in_channel=in_channel, width=1)
            self.ab_to_l = resnet101(in_channel=in_channel, width=1)
        else:
            raise NotImplementedError('model {} is not implemented'.format(name))
        self.modality = modality
        assert self.modality in ['touch','image']
...
class ResNet_concatref(pl.LightningModule):
    def __init__(self, name='resnet50', modality='touch', in_channel=3):
        super(ResNet_concatref, self).__init__()
        if name == 'resnet50':
            self.l_to_ab = resnet50(in_channel=in_channel, width=1)
            self.ab_to_l = resnet50(in_channel=in_channel, width=1)
        elif name == 'resnet18':
...
class MyResNetsCMC(pl.LightningModule):
    def __init__(self, name='resnet50v1', modality='touch', reference_length=1):
        super(MyResNetsCMC, self).__init__()
        if name.endswith('t2'):
            self.encoder = ResNetT2(name[:-2], modality)
        elif name.endswith('cat'):
            in_channel = 3 * reference_length
            self.encoder = ResNet_concat(name[:-3], modality=modality, in_channel=in_channel)
        elif name.endswith('cat_ref'):
            in_channel = 3 * reference_length
            self.encoder = ResNet_concatref(name[:-7], modality=modality, in_channel=in_channel)
        else:
